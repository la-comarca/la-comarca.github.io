...
const editorialCSS=await readFile('public/editorial-fix.css','utf8'),auditCSS=await readFile('public/visual-audit.css','utf8'),teamCSS=await readFile('public/team-page.css','utf8'),baseCSS=await readFile('dist/styles.css','utf8');
...
const version=createHash('sha256').update(await readFile('public/app.js')).update(await readFile('public/home-announcements.js')).update(await readFile('public/team-auth.js')).update(await readFile('public/styles.css')).update(editorialCSS).update(auditCSS).update(teamCSS).update(await readFile('public/calendar.js')).update(await readFile('public/motion.js')).update(await readFile('public/registration.js')).update(await readFile('public/formation.js')).update(await readFile('public/subscription.js')).digest('hex').slice(0,12);
