...
async function identity(env,accessToken){
 ...
 if(!profile||profile.auth_user_id!==user.id)throw Error('permission');
 ...
 const active=memberships.filter(item=>item.workspaces?.active!==false&&item.workspaces?.key).map(item=>({key:item.workspaces.key,name:item.workspaces.name||item.workspaces.key,description:item.workspaces.description||'',role:item.role,scope:item.scope||{}}));
