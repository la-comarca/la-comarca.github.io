...
// The service-role key never leaves the Worker.
...
const reply=(status,data,origin)=>new Response(JSON.stringify(data),{status,headers:{...JSON_HEADERS,...(origin?{'Access-Control-Allow-Origin':origin,'Access-Control-Allow-Credentials':'true','Vary':'Origin'}:{})}});
...
async function authFetch(env,path,{method='GET',body:payload,token,request}={}){
...
async function identity(env,accessToken,request){
 const response=await authFetch(env,'user',{token:accessToken,request});
 ...
 if(!profile)throw Error('permission');
 const memberships=await supabaseRequest(env,`rest/v1/workspace_memberships?user_id=eq.${encodeURIComponent(profile.id)}&active=eq.true&select=role,scope,workspaces(key,name,description,active)`);
 const active=memberships.filter(item=>item.workspaces?.active!==false).map(item=>({key:item.workspaces?.key,name:item.workspaces?.name||item.workspaces?.key,description:item.workspaces?.description||'',role:item.role,scope:item.scope||{}}));
...
export async function supabaseAuth(request,env){
 ...
 try{
  if(path==='/auth/token'&&request.method==='POST'){
   ...
   const v=await body(request);if(typeof v.email!=='string'||typeof v.password!=='string'||v.email.length>254||v.password.length>128||!/^\S+@\S+\.\S+$/.test(v.email))return reply(400,{message:'Revisa el correo y la contraseña.'},origin);
   const response=await authFetch(env,'token?grant_type=password',{method:'POST',body:{email:v.email.trim().toLowerCase(),password:v.password},request});
   const data=await response.json().catch(()=>({}));
   if(!response.ok||!data.access_token)return reply(401,{message:'No pudimos iniciar sesión. Revisa tus datos o confirma que tu cuenta de Supabase Auth esté activa.'},origin);
   try{const me=await identity(env,data.access_token,request);return reply(200,{access_token:data.access_token,refresh_token:data.refresh_token,expires_in:data.expires_in,token_type:data.token_type,user:me},origin);}catch(error){return reply(error.message==='permission'?403:401,{message:error.message==='permission'?'Tu cuenta existe, pero todavía no tiene acceso al equipo.':'No pudimos validar la sesión.'},origin);}
  }
  if(path==='/auth/refresh'&&request.method==='POST'){
   ...
   const response=await authFetch(env,'token?grant_type=refresh_token',{method:'POST',body:{refresh_token:v.refresh_token},request});const data=await response.json().catch(()=>({}));
   if(!response.ok||!data.access_token)return reply(401,{message:'Tu sesión terminó. Inicia sesión de nuevo.'},origin);
   const me=await identity(env,data.access_token,request);return reply(200,{access_token:data.access_token,refresh_token:data.refresh_token||v.refresh_token,expires_in:data.expires_in,token_type:data.token_type,user:me},origin);
  }
  if(path==='/auth/me'&&request.method==='GET'){
   const token=bearer(request);if(!token)return reply(401,{message:'Inicia sesión.'},origin);
   try{return reply(200,{user:await identity(env,token,request)},origin);}catch(error){return reply(error.message==='permission'?403:401,{message:error.message==='permission'?'Tu cuenta no tiene acceso al equipo.':'La sesión ya no es válida.'},origin);}
  }
  if(path==='/auth/logout'&&request.method==='POST'){
   const token=bearer(request);if(token)await authFetch(env,'logout',{method:'POST',token,request}).catch(()=>{});
