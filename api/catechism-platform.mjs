...
const catechismRole=me=>catechismModule(me)?.role||'';
const canWrite=me=>(roleRank[catechismRole(me)]||0)>=2;
...
const visible=(scope,groupId)=>scope===null||scope.has(groupId);
const visibleOnly=(scope,rows,key='group_id')=>scope===null?rows:rows.filter(row=>scope.has(row[key]));

async function accessScope(env,me){
 if(catechismRole(me)!=='catechist')return null;
 const profile=await currentProfile(env,me);if(!profile)return new Set();
 const catechist=(await supabaseRequest(env,`rest/v1/catechism_catechists?profile_id=eq.${profile.id}&active=eq.true&select=person_id&limit=1`))[0];
 if(!catechist)return new Set();
 const assignments=await supabaseRequest(env,`rest/v1/catechism_group_catechists?catechist_id=eq.${catechist.person_id}&active=eq.true&select=group_id`);
 return new Set(assignments.map(row=>row.group_id).filter(Boolean));
}

async function home(env,me,scope){
 const [ws,cycles,allGroups,students,catechists,enrollments,staff]=await Promise.all([
  workspace(env),
  ...
  supabaseRequest(env,'rest/v1/catechism_enrollments?status=in.(active,paused,pending)&select=student_id,group_id'),
  supabaseRequest(env,'rest/v1/catechism_group_catechists?active=eq.true&select=group_id,catechist_id')
 ]);
 const groups=scope===null?allGroups:allGroups.filter(group=>scope.has(group.id));
 const scopedEnrollments=visibleOnly(scope,enrollments),scopedStaff=visibleOnly(scope,staff);
 const studentIds=new Set(scopedEnrollments.map(row=>row.student_id)),catechistIds=new Set(scopedStaff.map(row=>row.catechist_id));
 const [from,to]=todayBounds();
 const sessionRows=await supabaseRequest(env,`rest/v1/catechism_sessions?starts_at=gte.${encodeURIComponent(from)}&starts_at=lt.${encodeURIComponent(to)}&select=id,title,kind,starts_at,ends_at,status,group_id,room_id&order=starts_at.asc`),sessions=visibleOnly(scope,sessionRows);
 let pendingDocuments=[];if(canWrite(me))pendingDocuments=await supabaseRequest(env,'rest/v1/catechism_student_documents?status=eq.pending&select=id,student_id,document_type,original_filename,uploaded_at&order=uploaded_at.asc&limit=8');
 return {workspace:ws,cycle:cycles[0]||null,metrics:{groups:groups.length,students:scope===null?students.length:studentIds.size,catechists:scope===null?catechists.length:catechistIds.size,pendingDocuments:pendingDocuments.length},today:sessions,pendingDocuments,groups:groups.slice(0,8)};
}

async function calendar(env,url,scope){
 ...
 const rows=await supabaseRequest(env,`rest/v1/catechism_sessions?starts_at=gte.${encodeURIComponent(from)}&starts_at=lt.${encodeURIComponent(to)}&select=id,title,kind,starts_at,ends_at,status,cancellation_reason,group_id,room_id,replaced_by_session_id&order=starts_at.asc`);
 return {sessions:visibleOnly(scope,rows)};
}

async function groups(env,scope){
 const [allRows,allEnrollmentCounts,allCatechists]=await Promise.all([
  supabaseRequest(env,'rest/v1/catechism_groups?select=id,name,level,weekday,starts_at,ends_at,active,room_id,cycle_id,program_id,catechism_cycles(name,status),catechism_programs(name),catechism_rooms(name,location)&order=name.asc'),
  supabaseRequest(env,'rest/v1/catechism_enrollments?status=in.(active,paused,pending)&select=group_id'),
  supabaseRequest(env,'rest/v1/catechism_group_catechists?active=eq.true&select=group_id,role,catechist_id')
 ]);
 const rows=scope===null?allRows:allRows.filter(row=>scope.has(row.id)),enrollmentCounts=visibleOnly(scope,allEnrollmentCounts),catechists=visibleOnly(scope,allCatechists);
...
async function groupDetail(env,id,scope){
 if(!uuid.test(id))throw Object.assign(Error('Grupo no válido.'),{status:400});if(!visible(scope,id))throw Object.assign(Error('Grupo no encontrado.'),{status:404});
...
async function students(env,url,scope){
 const q=clean(url.searchParams.get('q')||'',80).toLowerCase();
 const [people,allEnrollments]=await Promise.all([
  supabaseRequest(env,'rest/v1/catechism_students?select=person_id,status,joined_on,completed_on,baptism_date,baptism_place,catechism_people(first_name,last_name,preferred_name,birth_date,email,phone)&order=created_at.desc&limit=200'),
  supabaseRequest(env,'rest/v1/catechism_enrollments?status=in.(active,paused,pending)&select=id,student_id,group_id,status,enrolled_on,catechism_groups(name,level,cycle_id,program_id)&limit=400')
 ]);
 const enrollments=visibleOnly(scope,allEnrollments),byStudent=new Map();for(const row of enrollments){if(!byStudent.has(row.student_id))byStudent.set(row.student_id,[]);byStudent.get(row.student_id).push(row);}
 let result=people.filter(row=>scope===null||byStudent.has(row.person_id)).map(row=>({...row,enrollments:byStudent.get(row.person_id)||[]}));if(q)result=result.filter(row=>{const p=row.catechism_people||{};return `${p.first_name||''} ${p.last_name||''} ${p.preferred_name||''}`.toLowerCase().includes(q);});return {students:result};
}

async function studentDetail(env,id,me,scope){
 if(!uuid.test(id))throw Object.assign(Error('Alumno no válido.'),{status:400});
 const allEnrollments=await supabaseRequest(env,`rest/v1/catechism_enrollments?student_id=eq.${id}&select=id,status,enrolled_on,withdrawn_on,group_id,catechism_groups(name,level,catechism_cycles(name),catechism_programs(name))&order=enrolled_on.desc`),enrollments=visibleOnly(scope,allEnrollments);
 if(scope!==null&&!enrollments.length)throw Object.assign(Error('Alumno no encontrado.'),{status:404});
 ...
 const enrollmentIds=enrollments.map(row=>row.id),filter=enrollmentIds.join(',')||'00000000-0000-0000-0000-000000000000';
 const [contacts,progress,submissions,documents]=await Promise.all([
  ...
  supabaseRequest(env,`rest/v1/catechism_student_topic_progress?select=enrollment_id,topic_id,status,completed_at,evidence_type,source_session_id&enrollment_id=in.(${filter})`),
  supabaseRequest(env,`rest/v1/catechism_submissions?select=id,assignment_id,enrollment_id,status,submitted_at,points,feedback,graded_at,catechism_assignments(title,kind,max_points,due_at)&enrollment_id=in.(${filter})&order=updated_at.desc&limit=100`),
 ...
 const safeStudent=scope===null?student:{...student,pastoral_notes:'',catechism_people:{...(student.catechism_people||{}),notes:''}};
 return {student:safeStudent,enrollments,contacts,progress,submissions,documents};
}

async function catechists(env,scope){
 const [allRows,allAssignments]=await Promise.all([
  supabaseRequest(env,'rest/v1/catechism_catechists?select=person_id,profile_id,active,started_on,ended_on,formation_notes,catechism_people(first_name,last_name,preferred_name,email,phone)&order=created_at.desc'),
  supabaseRequest(env,'rest/v1/catechism_group_catechists?active=eq.true&select=group_id,catechist_id,role,catechism_groups(name,level)')
 ]);
 const assignments=visibleOnly(scope,allAssignments),visibleCatechists=new Set(assignments.map(row=>row.catechist_id)),rows=scope===null?allRows:allRows.filter(row=>visibleCatechists.has(row.person_id));
 ...
 return {catechists:rows.map(row=>({...row,...(scope===null?{}:{formation_notes:''}),groups:byCatechist.get(row.person_id)||[]}))};
}
...
export async function catechismPlatform(request,env){
 ...
 try{
  const scope=await accessScope(env,me);
  if(path==='/backoffice/v2/me'&&request.method==='GET')return reply(origin,200,{user:{id:me.id,name:me.name,email:me.email,role:catechismRole(me)},profile:await currentProfile(env,me)});
  if(path==='/backoffice/v2/home'&&request.method==='GET')return reply(origin,200,await home(env,me,scope));
  if(path==='/backoffice/v2/calendar'&&request.method==='GET')return reply(origin,200,await calendar(env,url,scope));
  if(path==='/backoffice/v2/groups'&&request.method==='GET')return reply(origin,200,await groups(env,scope));
  const groupRoute=path.match(/^\/backoffice\/v2\/groups\/([a-f0-9-]{36})$/i);if(groupRoute&&request.method==='GET')return reply(origin,200,await groupDetail(env,groupRoute[1],scope));
  if(path==='/backoffice/v2/students'&&request.method==='GET')return reply(origin,200,await students(env,url,scope));
  const studentRoute=path.match(/^\/backoffice\/v2\/students\/([a-f0-9-]{36})$/i);if(studentRoute&&request.method==='GET')return reply(origin,200,await studentDetail(env,studentRoute[1],me,scope));
  if(path==='/backoffice/v2/catechists'&&request.method==='GET')return reply(origin,200,await catechists(env,scope));
