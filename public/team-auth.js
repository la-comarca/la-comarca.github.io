...
if(root){
 ...
 const accessLabel=role=>({admin:'Administración',coordinator:'Coordinación',editor:'Edición',catechist:'Catequesis',reader:'Lectura'}[role]||'Acceso');
 const renderUser=user=>{
  form.hidden=true;panel.hidden=false;
  const modules=user.modules||[];
  panel.innerHTML=`<div class="team-session-head"><div><span class="eyebrow">TU EQUIPO</span><h2>Hola, ${esc(user.name||user.email)}.</h2><p>${esc(user.email)}</p></div><button class="button team-logout" type="button" data-team-logout><span>Cerrar sesión</span><span aria-hidden="true">↗</span></button></div><div class="team-module-list">${modules.map((module,index)=>`<article class="team-module-row"><small>${String(index+1).padStart(2,'0')}</small><div><span class="eyebrow">${esc(accessLabel(module.role))}</span><h3>${esc(module.name||moduleLabel(module.key))}</h3><p>${esc(module.description||'Herramientas y contenidos de este espacio.')}</p></div><span class="team-module-status">Disponible</span></article>`).join('')}</div>${modules.length?'':'<p class="team-empty">Todavía no hay secciones asignadas a esta cuenta.</p>'}`;
